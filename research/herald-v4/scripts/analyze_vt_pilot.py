"""Score the frozen variable-tracking population pilot without fitting a predictor."""
import argparse
import hashlib
import json
...
for item in run['prompts']:
 ...
 answers=r['manifest_row']['answers']; assert len(set(answers))==5
 ...
 loss=(sum(scores['reference']['pass_vector'])-sum(scores['action']['pass_vector']))/5
 assert abs(loss-(scores['reference']['score_fraction']-scores['action']['score_fraction']))<1e-12
...
checks={'mean_reference_at_least_090':bool(q.mean()>=.9),'at_least_12_perfect_references':bool(sum(q==1)>=12),'at_least_four_partial_losses':bool(sum((y>0)&(y<1))>=4),'at_least_three_loss_levels':len(set(y))>=3,'sample_std_at_least_010':bool(y.std(ddof=1)>=.1),'at_least_three_zero_losses':bool(sum(y==0)>=3),'at_least_three_positive_losses':bool(sum(y>0)>=3)}
