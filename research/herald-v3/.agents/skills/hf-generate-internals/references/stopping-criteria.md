...
### Timeout-Based Custom Stopping

A workflow that needs a timeout can define a criterion like this:
...
Returning a proper BoolTensor of shape `(batch_size,)` is the correct contract.
Keep the timeout policy separate from the batch shape so the criterion remains
safe when the workflow changes its batch size.
...
4. **MaxTimeCriteria is global, not per-sequence**: When time runs out, ALL
   sequences stop, so it cannot let only near-complete sequences continue.
