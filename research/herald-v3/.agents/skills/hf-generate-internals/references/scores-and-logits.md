...
This is the case for a call that uses greedy decoding without active custom
processors.
...
Use this when a workflow needs the full vocabulary distribution at each step
(for example, entropy, top-k, or KL divergence), because those calculations
require the full score tensor rather than only the selected token's score.
...
4. **With greedy + no custom processors, scores == logits**: Don't store both.
   Pick one based on whether post-processor or raw values are needed.
