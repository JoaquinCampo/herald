...
Reference for HF Transformers generation pipeline internals, covering the full
path from `model.generate()` to output tensors. Version-specific details
require local verification.

Portability note: Examples and version-specific details are references. Check
them against the chosen v3 environment before relying on them. This skill does
not commit the project to particular dependencies, models, or generation
methods.
