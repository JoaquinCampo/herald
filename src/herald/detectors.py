...
Currently detected: looping, non-termination, wrong_answer.

For hazard predictor training, use only looping + non_termination as labels
(true catastrophic behavior). wrong_answer is kept for evaluation — measuring
task accuracy alongside catastrophic behavior rate — but should NOT be used
as a training signal since getting math wrong is normal model behavior even
without compression.

Future work — additional catastrophe types not yet implemented:
- Instruction amnesia: model solves a different problem than asked.
