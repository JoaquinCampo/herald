"""Train and evaluate the per-segment K=16 onset predictor.

XGBoost on the per-segment dataset built by build_phase2_v2_segments.py.
5-fold GroupKFold by prompt_id. Reports AUROC + AUPRC per fold and
cluster-bootstrap CIs (resampling prompt_ids).

Strict online causality:
  - Drops `relative_progress` (uses post-hoc num_tokens_generated).
  - Adds `position_in_budget = seg_end_tok / max_budget` (observable
    online).
  - Drops onset columns and run-level metadata that aren't features.

Outputs:
  results/phase2_v2/xgb/{model_fold{i}.json, scores_fold{i}.parquet,
...
def main() -> None:
    ...
    ap.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/phase2_v2/xgb"),
    )
    ...
    logger.info("pos_rate={:.4%} scale_pos_weight={:.2f}", pos_rate, spw)
    ...
    for fold_i, (tr, te) in enumerate(gkf.split(X, y, groups)):
        ...
        pd.DataFrame(
            ...
            args.output_dir / f"scores_fold{fold_i}.parquet", index=False
        )
    ...
    logger.info(
        "AUROC mean={:.4f} std={:.4f}  AUPRC mean={:.4f} std={:.4f}",
        overall["auroc_mean"],
        overall["auroc_std"],
        overall["auprc_mean"],
        overall["auprc_std"],
